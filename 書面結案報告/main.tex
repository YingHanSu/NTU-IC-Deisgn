...
他建議我們再裝一個 Arduino NANO 版移出來，既不佔空間，也可以解決訊號延遲問題。
\vspace{1.5em}
...
修習這門課之前，我從未有過正式的工程訓練背景。從車體結構設計到機械手臂的製作與元件配置，每一個環節對我而言都是全新的挑戰。花在實驗室調試、修正、反覆嘗試的時間，讓我真實理解到工程設計不只是「找到答案」，而是在一次次失敗中不斷校正方向的過程。面對陌生的材料與工具，我漸漸學會如何觀察問題的根源、調整設計的邏輯，而不是一味套用既有的解法。這次跨領域的實作經驗，讓我意識到自己對動手設計與機械構造的濃厚興趣，也成為我進一步探索工程領域的重要起點。
