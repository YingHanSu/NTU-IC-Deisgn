...
表示法除了得顯示數字絕對值大小，還得有顯示數字正負的公用。例如，6 在二進位為 0110，若要表示 -6，第一思路是令 $\mathrm{(6)+(-6)=0}$。計算：
\[ \mathrm{(0110)+(1010)=(0000)} \]
但 1010 直接看是 +10。為了解決此問題，我們增加一個 \textbf{signed bit} (0 代表 +，1 代表 -)。一個 5-bit signed binary number 能表達 -16 到 +15。給定二進位正數 $\mathrm{A}$，其負數為 $\mathrm{(-A) = \text{NOT}(A) + 1}$。例如 $\mathrm{B = (0101110)}$，則 $\text{NOT}(\mathrm{B}) = \mathrm{1010001}$，故 $\mathrm{-B = 1010010}$。這即是 \textbf{2’s complement} 的概念。
...
\item \textbf{舉例：} 數位電路實現乘法，一開始我們會想像會做很多加法，畢竟乘法就是加法器的兒子。我們舉例去思考，$13 \times 11 = 143$，思考一個 4bit $\times$ 4bit Multiplier 如何設計？
$13 = (1101)$， $11 = (1011)$，以下為小學乘法器做法。若要節省加法器使用面積，可以用移位暫存器的方法，儲存加法進度，利用多個 cycle 把加法裡的從 LSB 到 MSB 一個個加完，如右下圖，省了很多面積/資源重複使用。也可以放很多加法器 array，會用很多加法器，但運算 1 個 cycle 就做完。
最後要提，現有乘法器 adder 因為要加很多 bit 數，從 LSB 加到 MSB 會有很長的 Ripple Add propagation path 導致 gate delay 很大，電路運算慢。所以我們會用 CLA (Carry Look Ahead) 的邏輯優化我們的加法器，讓他們跑得更快更有效率，利用了一點平行化的概念在裡面，平行化在乘法器設計裡扮演很重要角色。
\begin{figure}[H]
  \centering
  % 第一張圖
  \begin{subfigure}[b]{0.45\textwidth}
      \centering
      \includegraphics[width=0.8\textwidth]{figure/乘法器.drawio.png}
      \caption{原始乘法器架構}
      \label{fig:mul1}
  \end{subfigure}
  ...
  \begin{subfigure}[b]{0.45\textwidth}
      \centering
      \includegraphics[width=0.8\textwidth]{figure/乘法器改.drawio.png}
      \caption{優化後的乘法器架構}
      \label{fig:mul2}
  \end{subfigure}
...
\end{figure}
\item \textbf{指數運算 ($\mathrm{X^Y}$)：} 基本單位是乘法，使用 \textbf{square and multiply} 方法。將 $\mathrm{Y}$ 做二進位處理（如 $7^{19}$，將 19 轉為 $10011_2$），運算時間可從 $\mathrm{O(N)}$ 降至  $\mathrm{O(\log N)}$。此外，\textbf{CORDIC} 演算法利用歐拉公式知識，僅需加法器與暫存器配合 lookup tables 存雙曲函數值即可估計指數值。
\end{itemize}
