...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item 依據理論化簡後之電路圖進行佈線：利用 7486 負責處理總和 ($S$) 與部分進位邏輯，並將其輸出結果接入 7400 進行 NAND 邏輯運算，以產生最終的進位輸出 ($\mathrm{C_{out}})。
        \item 將 ADALM2000 的數位腳位 (DIO) 連接至電路對應節點：DIO 0 接至輸入 $A$、DIO 1 接至輸入 $B$、DIO 2 接至輸入 $C_{in}$；量測端的部分，將 DIO 3 接至總和輸出 $S$、DIO 4 接至進位輸出 $\mathrm{C_{out}}$。
    \end{itemize}
    \begin{center}
        \begin{figure}[H]
            \centering
