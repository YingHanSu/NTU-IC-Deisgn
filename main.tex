...
透過 ADALM2000 擷取訊號，我們記錄了 1-bit 全加器在 8 種不同輸入組合下的輸出結果（如表 \ref{table:truth_table} 所示）。分析該測量數據可得出以下結論：
    ...
    \end{center}
\begin{itemize}
    \item \textbf{總和 ($S$) 邏輯分析：} 當輸入端 $A, B, C_{in}$ 中有「奇數」個 1 處於高電位時（即狀態 3rd, 5th, 6th, 8th），總和輸出 $S$ 皆為 1。這完美吻合 XOR 閘 ($A \oplus B \oplus C_{in}$) 的奇偶校驗 (Parity Check) 特性。
    \item \textbf{進位 ($C_{out}$) 邏輯分析：} 當輸入端有「兩個或三個」 1 處於高電位時（即狀態 1st, 2nd, 3rd, 7th），進位輸出 $C_{out}$ 皆為 1。這證明了我們利用 IC 7400 (NAND 閘) 配合笛摩根定理所建構的進位電路，成功實現了全加器的多數決邏輯 (Majority Logic)。
\end{itemize}
