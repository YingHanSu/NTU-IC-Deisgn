...
加法器為數位邏輯電路中執行二進位加法運算之核心組件。最基本的\textbf{半加器}負責兩個單一位元 $\mathrm{A}$ 與 $\mathrm{B}$ 的加法，其輸出包含總和 ($\mathrm{S}$) 與進位 ($\mathrm{C_{out}}$)，布林邏輯表示式為：
\begin{gather}
...
\begin{enumerate}
    ...
    \begin{figure}[H]
        ...
        \caption{Fig. 1-bit 全加器電路佈線圖(這個圖有串接兩個 3-bit 加法器)}
        \label{fig:ic_layout}
    \end{figure}
...
運算過程觀察： 由於低位元（第1、2個 bit）的相加產生了進位，我們可以明確觀察到低位元模組的 $\mathrm{C_{out}}$ 輸出高電位，並成功觸發高位元模組的 $\mathrm{C_{in}}$ 接收端，最終正確點亮對應 $\mathrm{1000_2}$ （即數值 8）的輸出 LED 燈號。
